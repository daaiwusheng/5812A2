...
bool material::scatter(const ray &r_in, const hit_record &rec, color &alb, ray &scattered, double &pdf) const {
    return false;
}
...
diffuse_light::diffuse_light(color c) : emitmaterial(make_shared<solid_color>(c)) {}

bool
diffuse_light::scatter(const ray &r_in, const hit_record &rec, color &attenuation, ray &scattered, double &pdf) const {
    return false;
}
